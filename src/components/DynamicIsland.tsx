...
const DynamicAction = () => {
  ...
  const renderCompactState = () => (
    <DynamicContainer className="flex items-center justify-center h-full w-full">
      <div className="relative w-full flex items-center">
        <DynamicDescription className="absolute left-4  my-auto text-lg font-medium tracking-tighter text-black dark:text-white ">
          <MessageCircle className=" h-5 w-5 fill-cyan-400 text-cyan-400" />
        </DynamicDescription>

        <DynamicDescription className="absolute text-black dark:text-white right-4  my-auto text-lg font-bold tracking-tighter ">
          newcult.co
        </DynamicDescription>
      </div>
    </DynamicContainer>
  ...
  const renderLargeState = () => (
    <DynamicContainer className="flex items-center justify-center h-full w-full">
      <div className="relative  flex w-full items-center justify-between gap-6 px-4">
        ...
        <DynamicTitle className="my-auto text-2xl font-black tracking-tighter text-black dark:text-white ">
          loading
        </DynamicTitle>
      </div>
    </DynamicContainer>
  ...
  const renderTallState = () => (
    <DynamicContainer className="  flex flex-col mt-6 w-full items-start  gap-1 px-8 font-semibold">
      <DynamicDescription className="bg-cyan-300 rounded-2xl tracking-tight leading-5  p-2 text-black dark:text-white">
        The Cult of Pythagoras
      </DynamicDescription>
      <DynamicDescription className="bg-cyan-300 rounded-2xl tracking-tight leading-5  p-2 text-left text-black dark:text-white">
        Music of the Spheres, an idea that celestial bodies produce a form of
        music through their movements
      </DynamicDescription>

      <DynamicTitle className=" text-4xl font-black tracking-tighter text-cyan-200 black:text-cyan-400 dark:text-white">
        any cool cults?
      </DynamicTitle>
    </DynamicContainer>
  );
  ...
  const renderLongState = () => (
    <DynamicContainer className="flex items-center justify-center h-full w-full">
      <DynamicDiv className="relative  flex w-full items-center justify-between gap-6 px-4">
        ...
        <DynamicTitle className="my-auto text-xl font-black tracking-tighter text-black dark:text-white ">
  ...
  const renderMediumState = () => (
    <DynamicContainer className="flex flex-col justify-between px-2 pt-4 text-left text-neutral-800 dark:text-neutral-200 h-full">
      <DynamicTitle className="text-2xl pl-3 font-extrabold tracking-tight">
        Reincarnation, welcome back
      </DynamicTitle>
      <DynamicDescription className="leading-5 text-neutral-700 pl-3 dark:text-neutral-300">
        Good for small tasks or call outs
      </DynamicDescription>

      <DynamicDiv className="flex flex-col mt-auto space-y-1 mb-2 p-2 rounded-b-2xl">
        <Button>
          <Mail className="mr-2 h-4 w-4 text-cyan-400" /> Login
          with email
        </Button>
  ...
  const renderOtherStates = () => (
    <div className="flex items-center justify-center h-full w-full">
      <div>
        <ArrowUpLeftSquareIcon className="text-black dark:text-white" />
      </div>
      <p className="text-neutral-800 dark:text-neutral-200">cycle states</p>
    </div>
  );
